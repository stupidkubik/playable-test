...
const pagesServiceWorkerPath = path.join(pagesArtifactDir, "service-worker.js");
...
async function main() {
  ...
  await assertFileExists(distServiceWorkerPath, "npm run build");
  ...
  await fs.copyFile(distServiceWorkerPath, pagesServiceWorkerPath);
  await fs.writeFile(pagesNoJekyllPath, "", "utf8");

  console.log(`Prepared ${path.relative(rootDir, pagesIndexPath)} for GitHub Pages`);
  console.log(`Prepared ${path.relative(rootDir, pagesServiceWorkerPath)} for GitHub Pages`);
